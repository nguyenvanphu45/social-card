...
function ActionCard({ title, data, onClose }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!name.length || !description.length || !avatar) {
            setError(true);
        } else {
            !data
                ? dispatch(
            ...
            <form onSubmit={handleSubmit}>
                <div className={cx('form')}>
                    <div className={cx('upload')}>
                        <label className={cx('input-label', error && !avatar && 'error')}>
                            Avatar <span>*</span>
                        </label>
                        <div className={cx('input')}>
                            ...
                            <label
                                ...
                                className={cx('input-label', error && !avatar && 'error')}
                            >
                                <FaUpload className={cx('icon')} />
                                <p>{!avatar ? 'Upload image' : avatar}</p>
                            </label>
                        </div>
                    </div>
                    <div className={cx('upload')}>
                        <label className={error && !name.length ? cx('error') : ''}>
                            Name <span>*</span>
                        </label>
                        <div className={cx('input')}>
                            <input
                                type="text"
                                className={error && !name.length ? cx('input-error') : ''}
                                value={name}
                    ...
                    <div className={cx('upload')}>
                        <label className={error && !description.length ? cx('error') : ''}>
                        ...
                        <div className={cx('input')}>
                            <input
                                type="text"
                                value={description}
                                className={error && !description.length ? cx('input-error') : ''}
